...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const cleanDesc = (t) => {
        if (!t) return "Professional opportunity with a market leader. Join a high-performing team to build the future of India's industry.";
        let c = t.replace(/Role|Overview|Responsibilities|Requirements|Key/gi, "").trim();
        return c.length > 200 ? c.substring(0, 180) + "..." : c;
    };

    // Ultra-Neat Professional Rendering (Matches static cards)
    const renderJob = (job, isAdmin, API_BASE_URL) => {
        ...
        article.innerHTML = `
            ...
            <div class="job-meta-grid">
                <div class="meta-item"><i class="fas fa-map-marker-alt"></i> ${job.location || 'India'}</div>
            ...
            <div class="job-footer">
                ...
                <div style="display: flex; gap: 15px; align-items: center;">
                    ...
                    <a href="./apply_home.html?job_id=${job.job_id}" class="apply-btn">View Details</a>
                </div>
            </div>
        `;
        return article;
    ...
    window.adminDel = async (id, btn) => {
        ...
        try {
            const res = await fetch(`${getAPIURL()}/admin/jobs/${id}`, { method: "DELETE" });
            if (res.ok) btn.closest(".job-card").remove();
    ...
    const performSearch = () => {
        ...
        allCards.forEach(card => {
            ...
            const locationElem = card.querySelector('.meta-item');
            const locText = locationElem ? locationElem.innerText.toLowerCase() : fullText;
        ...
        if (count === 0) {
            if (!existingNoResults) {
                ...
                box.id = "noResultsBox";
                box.style.cssText = "text-align:center; padding:50px 0; color:#888;";
                box.innerHTML = `
                    <i class="fas fa-search" style="font-size:3rem; opacity:0.1; margin-bottom:15px;"></i>
    ...
    if (roleSearchInput) roleSearchInput.addEventListener("keyup", (e) => { if (e.key === "Enter") performSearch(); });
    if (locationSearchInput) locationSearchInput.addEventListener("keyup", (e) => { if (e.key === "Enter") performSearch(); });
    ...
    async function init() {
        try {
            ...
            if (res.ok) {
                ...
                filtered.forEach(j => {
                    jobsContainer.prepend(renderJob(j, isAdmin, API_BASE_URL));
                });
            }
