/**
 * Job Interactions - Handles Saving Jobs & Application Tracking
...
function toggleSaveJob(jobId, jobData) {
    ...
    if (index === -1) {
        ...
        savedJobs.push({ id: jobId, ...jobData, savedAt: new Date().toISOString() });
...
function saveApplicationLOCALLY(jobData) {
    ...
    if (!apps.some(a => a.id === jobData.id)) {
        apps.push({ ...jobData, status: 'Applied', appliedAt: new Date().toISOString() });
        localStorage.setItem(STORAGE_KEYS.APPLICATIONS, JSON.stringify(apps));
        showToast("Application Saved Locally", "success");
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    document.body.addEventListener('click', (e) => {
        ...
        if (btn) {
            ...
            let jobData = {};
            try {
                jobData = JSON.parse(decodeURIComponent(btn.dataset.job));
            } catch (err) {
                console.error("Invalid job data for ID:", jobId);
            }
